...
from llama_inference.llama import Llama, Tokenizer
...
class MiniImageNetDataset(Dataset):
    def __init__(self, data_root, image_size, time, model_path, vision_tokenizer, clip_texts, clip_codebook, text_token_embedding, way=2, shot=1, device="cpu", induction=1, codec = None, args = None, comet_experiment = None):

        self.comet_experiment = comet_experiment
        self.criterion_rd = RateDistortionLoss()
        self.device = device
        ...
        self.clip_codebook = clip_codebook

        self.induction = induction
    
        _clip, self.preprocess = clip.load("ViT-L/14", device=device)
        self.vision_tokenizer = vision_tokenizer
        self.clip_texts = clip_texts

        ## Load Codec and Adapter
        self.args = args
        self.codec   = codec[0]
        self.adapter = codec[1]
        if self.codec:
            self.codec.eval()
        if self.adapter:
            self.adapter.eval()
        self.Codec_preprocess = self._transform(_clip.visual.input_resolution)
        ...
        cells = {}
        for key in imagenet_dict.keys():
            cells[imagenet_dict[key][0]] = imagenet_dict[key][1]

        self.image_ids = []
        self.class_names = []
        with open("fewshot_split/%d_way_%d_shot.csv"%(way, shot)) as f:
            for line in f.readlines():
                ...
                for image_id in image_ids:
                    ...
                    cur_classes.append(cells[class_ind])
    ...
    def __getitem__(self, index):
        ...
        select_images = []
        select_images_clip = []
        query_classes = np.unique(self.class_names[index])
        ...
        Codec2CLIP_transform = transforms.Compose([
        ...
        with torch.no_grad():
            ...
            for i in range(0, len(image_ids)):
                ...
                image = Image.open(os.path.join(self.data_root, image_id)).convert('RGB')
                images_clip   = Codec2CLIP_transform(image)
                images_clip_all.append(images_clip)
                ...
                select_images.append(np.expand_dims(image, axis=0))
                select_texts.append(class_name)


            images = torch.from_numpy(np.concatenate(select_images, 0))
            images = images.to(self.device)
            
            images_clip_all = torch.stack(images_clip_all) 
            out     = self.codec(images_clip_all.to(self.device)) 

            assert (images_clip_all.shape[2], images_clip_all.shape[3]) == (256, 256), "Image size should be (256, 256)"
            num_pixels = images_clip_all.shape[0] * images_clip_all.shape[2] * images_clip_all.shape[3]
            ...
            query_rate = sum(
                (torch.log(likelihoods[-1]).sum() / (-math.log(2) * (images_clip_all.shape[2] * images_clip_all.shape[3])))
                for likelihoods in out["likelihoods"].values()
            )


            recon_image = out['x_hat'][-1]
            feature = self.adapter(out['y_hat'])     # [6, 256+1, 1024]
            _, d_idx_global_local = self.vision_tokenizer(images, feature, 0, step=0, is_val=True, k=self.args.token_num, start_layer=2)
            d_idx = d_idx_global_local[:, :self.args.token_num].contiguous()
            vis_texts_array = self.clip_texts[d_idx.view(-1).cpu().data].reshape(images.shape[0], -1)

        target_texts = select_texts[-1]

        ##Generating context examples with other images
        for i in range(0, self.args.repeat+1):
            for i in range(0, len(select_texts)):
                ...
                vis_texts = ""
                for text in vis_texts_array[i]:
                    vis_texts = vis_texts + text
                vis_token = torch.tensor(self.text_tokenizer.encode(vis_texts, bos=False, eos=False), dtype=torch.int64).to(self.device).unsqueeze(0)
        ...
        ##Adding query image token
        vis_texts = ""
        for text in vis_texts_array[-1]:
            vis_texts = vis_texts + text
        vis_token = torch.tensor(self.text_tokenizer.encode(vis_texts, bos=False, eos=False), dtype=torch.int64).to(self.device).unsqueeze(0)
        ...
        # print(f"Prompt tokens: {len(prompt_tokens)}, prompt_features: {prompt_features.shape}, max_len: {max_len}")
        if recon_image is None:
            recon_image = torch.tensor(0)

        return [prompt_tokens, prompt_features, target_texts, out_mask, max_len, image_ids, rate, select_texts, recon_image, query_rate]
    
imagenet_dict = {"0": ["n01440764", "tench"], "1": ["n01443537", "goldfish"], "2": ["n01484850", "great_white_shark"], "3": ["n01491361", "tiger_shark"], "4": ["n01494475", "hammerhead"], "5": ["n01496331", "electric_ray"], "6": ["n01498041", "stingray"], "7": ["n01514668", "cock"], "8": ["n01514859", "hen"], "9": ["n01518878", "ostrich"], "10": ["n01530575", "brambling"], "11": ["n01531178", "goldfinch"], "12": ["n01532829", "house_finch"], "13": ["n01534433", "junco"], "14": ["n01537544", "indigo_bunting"], "15": ["n01558993", "robin"], "16": ["n01560419", "bulbul"], "17": ["n01580077", "jay"], "18": ["n01582220", "magpie"], "19": ["n01592084", "chickadee"], "20": ["n01601694", "water_ouzel"], "21": ["n01608432", "kite"], "22": ["n01614925", "bald_eagle"], "23": ["n01616318", "vulture"], "24": ["n01622779", "great_grey_owl"], "25": ["n01629819", "European_fire_salamander"], "26": ["n01630670", "common_newt"], "27": ["n01631663", "  "], "28": ["n01632458", "spotted_salamander"], "29": ["n01632777", "axolotl"], "30": ["n01641577", "bullfrog"], "31": ["n01644373", "tree_frog"], "32": ["n01644900", "tailed_frog"], "33": ["n01664065", "loggerhead"], "34": ["n01665541", "leatherback_turtle"], "35": ["n01667114", "mud_turtle"], "36": ["n01667778", "terrapin"], "37": ["n01669191", "box_turtle"], "38": ["n01675722", "banded_gecko"], "39": ["n01677366", "common_iguana"], "40": ["n01682714", "American_chameleon"], "41": ["n01685808", "whiptail"], "42": ["n01687978", "agama"], "43": ["n01688243", "frilled_lizard"], "44": ["n01689811", "alligator_lizard"], "45": ["n01692333", "Gila_monster"], "46": ["n01693334", "green_lizard"], "47": ["n01694178", "African_chameleon"], "48": ["n01695060", "Komodo_dragon"], "49": ["n01697457", "African_crocodile"], 
...
def get_args_parser(argv):
    ...
    parser.add_argument("--mini_imagenet_path", default="/data/all", type=str, help="path of llama model")
    ...
    parser.add_argument("--stage_1_ckpt", type=str, default="stage_1_llama_fix-40.pth", help="Decoding Loss")
    parser.add_argument("--stage_2_ckpt", type=str, default="stage_2_llama_fix-10.pth", help="Decoding Loss")
    parser.add_argument("--embed_dim", type=int, default=4096, help="Decoding Loss")
    ...
    parser.add_argument("--repeat", type=int, default=0, help="Decoding Loss")
    parser.add_argument("--induction", type=int, default=1, help="Decoding Loss")
    parser.add_argument("--token_num", type=int, default=1, help="Decoding Loss")
    ...
    parser.add_argument("--global_embedding_path", default="clip_codebook_1st_2nd_3rd_filtering.pth", type=str, help="path of llama model")
    parser.add_argument("--local_embedding_path", default="clip_codebook_1st_2nd_3rd_filtering.pth", type=str, help="path of llama model")
...
def main(argv):
    ...
    ###Load Codebook Vocabulary and Codebook Embedding
    clip_texts = np.load(args.clip_texts, allow_pickle=True)
    clip_codebook = torch.load(args.clip_codebook).to(args.device)
    ...
    net = image_models[args.model](quality=int(ql))
    net = net.to(device)

    adapter = Linear_Encoder(in_features=320, out_features=512, num_tokens=1, args=args)
    ...
    experiment.set_name(f'V2L_{args.way}way_{args.shot}shot_{args.exp_name}_{args.quality_level}')
    Hyperparameters = vars(args)
    Hyperparameters["location"] = os.getlogin()
    experiment.log_parameters(Hyperparameters)
    experiment.add_tags([Hyperparameters["location"]])
    ...
    ###Load Dataset
    output_dir = f'{args.root}/V2L_{args.way}way_{args.shot}shot_{args.exp_name}/{args.quality_level}/'
    os.makedirs(output_dir, exist_ok=True)


    dataset = MiniImageNetDataset(
        data_root=args.mini_imagenet_path, image_size=args.image_size, time=0, clip_texts=clip_texts, clip_codebook=clip_codebook, model_path=args.llama_model_path, vision_tokenizer=model, text_token_embedding=generator.model.tok_embeddings, way=args.way, shot=args.shot, device=device, induction=args.induction, codec = codec, args = args, comet_experiment = experiment
    )
    ...
    result_file = open(os.path.join(output_dir, f'result_{args.way}_way_{args.shot}_shot.csv'), 'w')
    print(f"result_file: {os.path.join(output_dir, f'result_{args.way}_way_{args.shot}_shot.csv')}")
 
    rate_avg = AverageMeter()
    tqdm_emu = tqdm.tqdm(enumerate(data_loader), total=len(data_loader), leave=False, ascii=True)
    for data_iter_step, [prompt_tokens, prompt_features, target_texts, out_mask, max_lens, image_ids, rate, select_texts, recon_image, query_rate] in tqdm_emu:
        ...
        predictions = generator.generate_fewshot(
            prompt_tokens,
            prompt_features,
            induction=args.induction, #No induction settings do not use out_mask
            out_mask = out_mask,
            max_gen_len=16,
            temperature=0,
            top_p=1.0,
        ...
        for target_text, prediction in zip(target_texts, predictions):
            ...
            print(f"select_texts: {select_texts}")
            print(f"Ans: {pred_text}\n")
            print(f"pred tokens: {pred}\n")

            print("Prediction: %s, Ground Truth: %s, Acc: %.4f, Rate: %.4f\n"%(pred_text[:len(target_text)], target_text, acc / time, rate_avg.avg))
        
        # metric_logger.update(Accuracy=acc / time)
        rate_avg.update(rate)

        log = {
            'accuracy': acc / time,
            'bpp':  rate_avg.avg
        }
        experiment.log_metrics(log, step=time)

        if time % 2000 == 0: #Save Acc and Times for Resuming
            save_np = {}
            save_np["acc"] = acc
            save_np["time"] = time
            np.save(os.path.join(output_dir, "%d-k_%d-way_%d-shot_%d-repeat_%d-induction.npy"%(args.token_num, args.way, args.shot, args.repeat, args.induction)), save_np)


        for img_id in image_ids:
            for img_shot in img_id:
                result_file.write(f"{img_shot},")
        for select_text in select_texts:
            for text_shot in select_text:
                result_file.write(f"{text_shot},")
        
        result_file.write(f"{pred_text},")
    ...
    experiment.log_code(os.path.join(output_dir, f'result_{args.way}_way_{args.shot}_shot.csv'))
    print("Accuracy: ", acc / time)
    with open(os.path.join(output_dir, "few_shot.csv"), 'w') as f:
        f.write("%d, %d, %d, %f, %f \n"%(args.way, args.shot, time, acc / time, rate_avg.avg))
        f.flush()
