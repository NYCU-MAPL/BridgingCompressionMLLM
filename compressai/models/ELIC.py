...
class ELIC_LLM(CompressionModel):

    def __init__(self, N=192, M=320, num_slices=5, trans=None):
    ...
    def forward(self, x, noisequant=False):
        ...
        if self.llm_trans_encoder is not None:
            llm_emb = self.llm_trans_encoder(y_hat, (H, W)) # [B, 256, 4096]
        else:
            llm_emb = None
